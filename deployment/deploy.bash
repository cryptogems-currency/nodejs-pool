...
cd ~
git clone https://github.com/cryptogems-currency/nodejs-pool.git  # Change this depending on how the deployment goes.
cd /usr/src/gtest
sudo cmake .
sudo make
...
cd /usr/local/src
sudo git clone https://github.com/cryptogems-currency/cryptogem.git
cd cryptogem
...
sudo make -j$(nproc)
sudo cp ~/nodejs-pool/deployment/cryptogem.service /lib/systemd/system/
sudo useradd -m cryptogemdaemon -d /home/cryptogemdaemon
#BLOCKCHAIN_DOWNLOAD_DIR=$(sudo -u cryptogemdaemon mktemp -d)
#sudo -u cryptogemdaemon wget --limit-rate=50m -O $BLOCKCHAIN_DOWNLOAD_DIR/blockchain.raw https://downloads.getmonero.org/blockchain.raw
#sudo -u cryptogemdaemon /usr/local/src/cryptogem/build/release/bin/monero-blockchain-import --input-file $BLOCKCHAIN_DOWNLOAD_DIR/blockchain.raw --batch-size 20000 --database lmdb#fastest --verify off --data-dir /home/monerodaemon/.bitmonero
#sudo -u cryptogemdaemon rm -rf $BLOCKCHAIN_DOWNLOAD_DIR
sudo systemctl daemon-reload
sudo systemctl enable cryptogem
sudo systemctl start cryptogem
